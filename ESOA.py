...
# In[]
class ESOA:
    def __init__(self, func, n_dim, size_pop, max_iter, lb, ub, constraint_ueq=None, rand = 20):
        ...
        self.y_global_best = self.func(self.x[0])
        
        self.hop = self.ub - self.lb
        
        self.test_acc = 0
        self.best_para = self.x_global_best
        self.best_g    = self.y_global_best
        self.alpha = 1
        self.rand = rand
    ...
    def randomSearch(self):
        ...
        x_n = self.x + np.tan(r) * self.hop/( 1 + self.rand) *0.5 *self.alpha
    ...
    def adviceSearch(self):
        x_o = self.x + np.exp(-self.times/(0.1*self.rand))  * self.hop * self.g * self.alpha#np.exp(-self.times/(0.1*self.rand)) * 0.1 * self.hop * self.g * self.alpha
    ...
    def run(self):
        ...
        for epoch in range(self.max_iter):
            ...
            for self.times in range(self.rand):
                ...
                if self.times % 20 ==0:
                    val = self.df.validation(self.x_global_best)
                    
                    print("ind:", epoch ,"T:", val)

                    if val > self.test_acc:
                        print("Accespt")
                        self.test_acc = val
                        self.best_para = self.x_global_best.copy()
                        self.best_g    = self.g_global_best.copy()
                    
                    else:
                        self.x_global_best = self.best_para.copy()
                        self.g_global_best = self.best_g.copy()
                        print(self.x_global_best)
                        #print(self.g_global_best)
